...
def save_points_and_labels_to_txt(points: np.ndarray, labels: np.ndarray, filename='output.txt'):
    ...
    # 保存到文件
    np.savetxt(os.path.join('results', filename), points_and_labels_np, fmt='%.6f', delimiter=' ', comments='')
...
def main(args):
    ...
    model_name = os.listdir('/Users/lee/GitProjects/fork-pointnet2-pytorch/detection_model_use/pointnet2_part_seg_msg_usemac_welding/logs')[0].split('.')[0]
    ...
    # 加载保存的最佳模型检查点
    checkpoint = torch.load('/Users/lee/GitProjects/fork-pointnet2-pytorch/detection_model_use/pointnet2_part_seg_msg_usemac_welding/checkpoints/best_model.pth')
